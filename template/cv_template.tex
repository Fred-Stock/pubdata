...
\resumeSubHeadingListStart
<% for s in service: %>
  <% set label = s.service_type.lower() == 'reviewer' and 'Reviewer' 
...
In the above have <<collabs[0].total>> collaborators:
<<collabs[0].collaborators>>
...
<% for r in references:%>
\resumeSubheading{<< r.writer >>}{<< r.position >>}{<< r.email >>}{<< r.affiliation >>}    
<% endfor %>
